...
::: {.callout-tip}
If your organization does not have an existing RedHat account, you can create a RedHat trial account for a temporary OCP deployment (60 days). Instructions [here](01-prereqs.html#redhat-ocp-account-and-pull-secret) under the expandable section "Obtain a RedHat Trial Account"
:::
...
The Red Hat pull secret must be downloaded from [here](https://console.redhat.com/openshift/downloads#tool-pull-secret). 
...
::: {.callout-note}
This template assumes that roles `BootNodeRole` and `DevOps-Role` already exist.
