...
::: {.callout-note}
This section is not complete and under construction.
:::
...
```bash
git clone https://github.com/ibm-client-engineering/solution-journal-quantum-safe.git
```

Go to the opa library templates directory.

```bash
cd solution-journal-quantum-safe/opa/library/templates
```

Apply the templates using `-k` option (kustomize).
...
```bash
cd solution-journal-quantum-safe/opa/library/constraints
```
