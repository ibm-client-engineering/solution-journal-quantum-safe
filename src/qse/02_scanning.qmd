...
GitHub repo with code samples found [here](https://github.com/kevinshah-ibm/qse-samples)
...
::: {.callout-note}
Run Cryptography Analysis BEFORE running API Discovery. This will avoid file overwriting and keep the results of the API discovery scan intact. 

Additionally, Cryptography Analysis is currently only available on Java. See [here](https://www.ibm.com/docs/en/quantum-safe/quantum-safe-explorer/2.x?topic=quantum-safe-explorer-overview#languages-and-cryptography-libraries-supported) for up-to-date list of languages and libraries supported
:::

#### Cryptography Analysis Scan

Before we run the Java cryptography analysis, make sure the Java files are compiled by running `javac VulnerableCryptoExample.java` and `javac WeakCryptoVulnerabilities.java` in the java_files subfolder.

![](/images/scanning-crypto)

First, let's click on "Scan Cryptography Analysis"

If it's your first time running a cryptography analysis in the repo, you will see the following input box for the class path:

![](/images/scanning-class-path)

For the class path, put in the path where the .class files are located and press Enter

In my directory, the class path is: `c:\\Users\\itzuser\\Desktop\\qse-samples\\sample_files\\java_files`

If you have run a cryptography analysis in this repo before, you can edit the `.vscode\settings.json` file to specify the class path with the compiled Java files.

Here is an example of a working settings.json file:

```bash
{
    "qs-explorer.classPath": [
        {
            "path": "c:\\Users\\itzuser\\Desktop\\qse-samples\\sample_files\\java_files",
            "workspace": "c:\\Users\\itzuser\\Desktop\\qse-samples\\sample_files"
        }
    ]
}
```
...
We should now see the EXPLORER SCAN RESULTS tab populated with the Java Cryptographic Analysis

![](/images/cryptographic_results.png)

#### API Discovery Scan

![](/images/scanning4.png)

Now, let's click on "Scan API discovery"
...
![](/images/macos-qse-cli-params1.png)

![](/images/macos-qse-cli-params2.png)
...
**API Discovery Scan (Multiple Languages):**

`./cli.sh -i <package-path> -l ".go,.dart,.java,.jar,.py,.cpp,.cs"`

**Cryptography Analysis Scan (Ensure Java Files are Compiled before Running):**
...
**Remember:** Run Cryptography Analysis BEFORE running API Discovery. This will avoid file overwriting and keep the results of the API discovery scan intact.
:::
...
![](/images/scan-results-directory.png)
...
## IBM Quantum Safe Explorer Insights

**Accessing IBM Quantum Safe Explorer Insights**

To launch the Insights browser:

1. Navigate to the extracted CLI folder
2. Locate the `insights` folder and click on `index.html` to open the page in your preferred web browser
3. Click on "Open folder" and navigate to the `sample_files` repo where the folder `qs_explorer_result` is located
4. Upload the entire `qs_explorer_result` folder
5. Select any of the .json files to view the Analysis and Discovery scan results.

All functionalities on this page are consistent with those available in the IBM Quantum Safe VS Code extension.
