...
The first step to enabling this is to follow the [recommended fixes](/src/qse/04_github_actions_integration.html#upload-artifact-andor-downloads) for the `upload-artifact` issue, which *sends the scan results from the Github Action directly to the machine which hosts the Portfolio View dashboard*.
...
3. Run `npm start` in the apm-dataloader directory to load the data.

See the [instructions on data loading](/src/qse/05_portfolio_view.html#loading-the-explorer-scan-output-into-the-database) for more details on the Portfolio View data ingestion process.
...
![Github Repository secrets](/images/qse-github-actions-repo-secrets.png)
...
a) For *accessing via an identity file* (key):
